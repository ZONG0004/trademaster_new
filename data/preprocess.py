...
def generate_normalized_feature(df):
    ...
    for i in range(len(unique_ticker)):
        ...
        temp_indicator["zclose"] = (
            temp_indicator.close/(temp_indicator.close.rolling(2).sum()-temp_indicator.close))-1
        temp_indicator["zd_5"] = (temp_indicator.adjcp.rolling(
            5).sum()/5)/temp_indicator.adjcp-1
        temp_indicator["zd_10"] = (temp_indicator.adjcp.rolling(
            10).sum()/10)/temp_indicator.adjcp-1
        temp_indicator["zd_15"] = (temp_indicator.adjcp.rolling(
            15).sum()/15)/temp_indicator.adjcp-1
        temp_indicator["zd_20"] = (temp_indicator.adjcp.rolling(
            20).sum()/20)/temp_indicator.adjcp-1
        temp_indicator["zd_25"] = (temp_indicator.adjcp.rolling(
            25).sum()/25)/temp_indicator.adjcp-1
        temp_indicator["zd_30"] = (temp_indicator.adjcp.rolling(
            30).sum()/30)/temp_indicator.adjcp-1
